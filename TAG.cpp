...
PROCESS_STEPS postgame_menu(sf::RenderWindow& window, sf::RectangleShape& background, Player& player, std::error_code& syst_error, GAME_RESULT& game_res) {
		...
		if (game_res != GAME_RESULT::PLAYER_GAVE_UP) {
			init_text(time_res, window.getSize().x / 2, window.getSize().y / 4, L"Вы справились за: " + (sf::String)player.get_strtime_res(), 40,
...
//Примечания: конструкторы копирования не используются, т.к. достаточно тех, что будут созданы по умолчанию, т.к. нет явного динамического выделения памяти
