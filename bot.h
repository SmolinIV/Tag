...
Bot::Bot(Board& board) : 
	Bot() {
	b_board = board; // “.к. как нет работы с дин. пам€тью, компил€тор и сам создаст отличный конструктор копировани€
}
...
// «адание "точки старта" дл€ сборки - поиск кубика с цифрой 1, определение его должной позиции (далее - "дом") пустого пространства 
...
// ѕоиск следующего кубика и обновление данных "дома"
...
void Bot::assemble_board(sf::RenderWindow& window) {
	...
	// —борка всех р€дов кроме двух последних
	if (cp_home_pos.i < b_side_size - 2) {
		if (cp_home_pos.j < b_side_size - 1) {
			...
			//—лучай, когда пустота справа и под ней нужное число
